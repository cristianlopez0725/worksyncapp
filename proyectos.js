...
document.addEventListener("DOMContentLoaded", function () {
    const exportButton = document.getElementById("btnExportarArchivos");
    ...
    if (exportButton) {
        exportButton.addEventListener("click", () => {
            ...
            let contenido = `Detalles de la Tarea\n\n`;
            contenido += `Título: ${tarea.titulo}\n`;
            contenido += `Descripción: ${tarea.descripcion || "Sin descripción"}\n`;
            contenido += `Lista: ${proyectos[proyectoActivo].listas[listaActual].titulo}\n`;

            if (tarea.etiquetas.length > 0) {
                contenido += `Etiquetas: ${tarea.etiquetas.map(e => e.texto).join(", ")}\n`;
            }

            if (tarea.miembros.length > 0) {
                contenido += `Miembros: ${tarea.miembros.join(", ")}\n`;
            }

            contenido += `Fecha de inicio: ${tarea.fechas.inicio || "No definida"}\n`;
            contenido += `Fecha de vencimiento: ${tarea.fechas.vencimiento || "No definida"}\n`;

            doc.text(contenido, 10, 10);
            doc.save(`${tarea.titulo}.pdf`);
        });
    }
});
